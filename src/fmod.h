...
#define FSOUND_MPEGACCURATE                                                                                                                                 \
	0x00020000 /* For FSOUND_Stream_OpenFile - for accurate FSOUND_Stream_GetLengthMs/FSOUND_Stream_SetTime.  WARNING, see FSOUNDStream_OpenFile for inital \
				  opening time performance issues. */
...
#define FSOUND_ENABLEFX                                                                                                                                       \
	0x00100000 /* Allows DX8 FX to be played back on a sound.  Requires DirectX 8 - Note these sounds cannot be played more than once, be 8 bit, be less than \
				  a certain size, or have a changing frequency */
...
#define FSOUND_GEOMETRY_OPENING_REFERENCE \
	0x04 /* Overlays a transparency over the previous polygon.  The openingfactor supplied is pointed to (for access when building a list) */
/* [DEFINE_END] */
...
DLL_API void F_API FSOUND_File_SetCallbacks(unsigned int(_cdecl *OpenCallback)(const char *name), void(_cdecl *CloseCallback)(unsigned int handle),
...
DLL_API signed char F_API FSOUND_FX_SetDistortion(int channel, float Gain, float Edge, float PostEQCenterFrequency, float PostEQBandwidth,
...
DLL_API signed char F_API FSOUND_FX_SetFlanger(int channel, float WetDryMix, float Depth, float Feedback, float Frequency, int Waveform, float Delay,
											   int Phase);
DLL_API signed char F_API FSOUND_FX_SetGargle(int channel, int RateHz, int WaveShape);
DLL_API signed char F_API FSOUND_FX_SetI3DL2Reverb(int channel, int Room, int RoomHF, float RoomRolloffFactor, float DecayTime, float DecayHFRatio,
												   int Reflections, float ReflectionsDelay, int Reverb, float ReverbDelay, float Diffusion, float Density,
												   float HFReference);
...
DLL_API signed char F_API FSOUND_Geometry_Material_SetAttributes(FSOUND_MATERIAL *material, float reflectancegain, float reflectancefreq,
																 float transmittancegain, float transmittancefreq);
DLL_API signed char F_API FSOUND_Geometry_Material_GetAttributes(FSOUND_MATERIAL *material, float *reflectancegain, float *reflectancefreq,
																 float *transmittancegain, float *transmittancefreq);
...
FSOUND_Reverb_SetEnvironmentAdvanced(int env, int Room,			 /* [-10000, 0]     default: -10000 mB	or use FSOUND_REVERB_IGNOREPARAM */
...
DLL_API signed char F_API FSOUND_Reverb_GetEnvironmentAdvanced(int *env, int *Room, int *RoomHF, float *RoomRolloffFactor, float *DecayTime,
															   float *DecayHFRatio, int *Reflections, float *ReflectionsDelay, int *Reverb, float *ReverbDelay,
															   float *EnvironmentSize, float *EnvironmentDiffusion, float *AirAbsorptionHF);
