...
void EnnemiSnorkInBlack::onAvance() {
	...
	if (dir == SENS_GAUCHE) {
		...
		if (mur_opaque(x - SNORK_IN_BLACK_SPEED, y) || (x - SNORK_IN_BLACK_SPEED < xmin)) dir = SENS_DROITE;
	} else {
		marche(SNORK_IN_BLACK_SPEED);

		pic = pbk_ennemis[anime(anim_snork_in_black_marche_droite, 4, 4)];

		if (mur_opaque(x + SNORK_IN_BLACK_SPEED, y) || (x + SNORK_IN_BLACK_SPEED > offset + 640)) dir = SENS_GAUCHE;
	}
